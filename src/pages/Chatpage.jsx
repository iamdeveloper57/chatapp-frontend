...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    s.on("privateMessage", (msg) => {
      if (activeUser && (msg.sender === activeUser._id || msg.receiver === activeUser._id)) {
        setMessages((prev) => [...prev, msg]);
        scrollToBottom();
      }
    });

    s.on("call-made", ({ from, offer }) => setIncomingCall({ from, offer }));
  ...
  const selectUser = async (user) => {
    setActiveUser(user);
    const res = await fetch(`${API_URL}/messages/${user._id}`, { headers: { Authorization: `Bearer ${token}` } });
    const oldMessages = await res.json();
    setMessages(oldMessages);
    setTimeout(scrollToBottom, 100);
  };
  ...
  const createPeerConnection = (toId) => {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) socket.emit("ice-candidate", { to: toId, candidate: event.candidate });
    };
  ...
  return (
    <div className="h-screen w-screen flex bg-gray-50">
      {/* Sidebar */}
      <div className="flex flex-col w-1/3 max-w-[320px] border-r bg-white">
        <div className="p-4 font-bold border-b text-lg">Chats</div>
        <div className="flex-1 overflow-y-auto">
          {users.map((u) => (
            <div key={u._id} className="p-3 cursor-pointer flex justify-between items-center hover:bg-gray-100">
              <span onClick={() => selectUser(u)}>{u.username}</span>
              {onlineUsers.includes(u._id) && <span className="text-green-500">●</span>}
              <button onClick={() => startCall(u._id)} className="ml-2 text-blue-600">📞</button>
            </div>
          ))}
        </div>
        <button onClick={handleLogout} className="m-3 p-2 bg-red-500 text-white rounded">Logout</button>
      </div>

      {/* Chat Window */}
      <div className="flex-1 flex flex-col h-screen">
        <div className="sticky top-0 z-10 p-4 bg-blue-600 text-white flex items-center">
          <span className="text-lg font-semibold">{activeUser?.username || "Select a chat"}</span>
        </div>
          ...
          {messages.map((m, i) => {
            ...
            return (
              <div key={i} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                <div className={`px-3 py-2 max-w-[75%] break-words ${isMe ? "bg-sky-400 text-white rounded-l-2xl rounded-tr-2xl rounded-br-none" : "bg-yellow-800 text-white rounded-r-2xl rounded-tl-2xl rounded-bl-none"}`}>
                  {m.text}
                  <div className="text-[10px] text-gray-100 text-right mt-1">{new Date(m.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="sticky bottom-0 z-10 flex p-3 border-t bg-white">
          <input type="text" className="flex-1 border rounded-full px-4 py-2" value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} />
          <button onClick={sendMessage} className="ml-2 bg-blue-600 text-white px-4 py-2 rounded-full">Send</button>
        </div>
      </div>
      ...
      {incomingCall && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-4 rounded shadow text-center">
            <p>Incoming call from {users.find(u => u._id === incomingCall.from)?.username}</p>
            <button onClick={answerCall} className="bg-green-500 text-white px-4 py-2 rounded mr-2">Answer</button>
            <button onClick={() => setIncomingCall(null)} className="bg-red-500 text-white px-4 py-2 rounded">Decline</button>
          </div>
        </div>
      )}
    </div>
