...
export default function Chat() {
  ...
  const answerCall = async () => {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) socket.emit("ice-candidate", { to: incomingCall.from, candidate: event.candidate });
    };
    ...
    <div className="h-screen w-screen flex bg-gray-50">
      ...
      {(!isMobile || (isMobile && mobileView === "chat")) && activeUser && (
        <div className="flex-1 flex flex-col h-screen">
          ...
          {/* Messages */}
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-4 py-2 flex flex-col gap-2 bg-gray-50">
            {messages.map((m, i) => {
              const senderId = m.sender.toString();
              const isMe = senderId === myId;
              return (
